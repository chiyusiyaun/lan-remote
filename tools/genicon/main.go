...
// Generates distinct ICOs: client (remote monitor) and server (hub/node).
import (
	"bytes"
	"encoding/binary"
	"image"
...
// Client: cyan/blue monitor + link arrows
...
// Server: tower / hub with green status LED and radiating links
...
func build(name string, draw func(int) *image.NRGBA) {
	...
	println("wrote", name, len(data))
}

func main() {
	build("icon-client.ico", drawClient)
	build("icon-server.ico", drawServer)
}
